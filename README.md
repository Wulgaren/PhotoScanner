...
**iCloud Optimize:** when originals are not on disk, Train falls back to real review thumbs in `.cache/review_thumbs/` (matched via scan/session UUID→path). Placeholder `missing_*.jpg` files are never used. Full-res BadPhotos are downscaled to the same max edge (900) before embedding so goods and bads share one domain. Rebuild the feature cache after this change (Train clears it automatically when the preprocess tag changes).
